...
import { LokiS3BucketConstruct } from '../../src/constructs/loki-s3-bucket';
import {
  createTestConfig,
  synthesizeChart,
  findResource,
...
describe('LokiS3BucketConstruct', () => {
  it('should create a Bucket resource', () => {
    ...
    // Act
    new LokiS3BucketConstruct(chart, 'test-bucket', { config });
    ...
    expect(bucket.metadata.name).toBe(config.s3.buckets.loki);
  });
  ...
  it('should configure 90-day retention for logs', () => {
    // Arrange
    const chart = Testing.chart();
    const config = createTestConfig();
    ...
    expect(rules[0].id).toBe('delete-old-logs'); // S3 lifecycle action: delete
    expect(rules[0].status).toBe('Enabled');
    expect(rules[0].expiration[0].days).toBe(90); // 3 months
  });
